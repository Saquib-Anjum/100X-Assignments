...
**Tech Stack:** Node.js, Express, PostgreSQL, Prisma, Zod, JWT, bcrypt, `ws` (WebSocket)
...
- Attendance persistence to PostgreSQL (via Prisma)
...
```jsx
{
  _id: String (UUID),
  className: String,
  teacherId: String (UUID), // reference to User
  studentIds: [String (UUID)] // array of User references
}
```
...
**Note:** Check PostgreSQL Attendance table (via Prisma) for persisted record
...
2. **Get all students in active class from PostgreSQL via Prisma:**
